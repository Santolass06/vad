...
3. **Revisão pós-fecho (mesma sprint):** encontrados e corrigidos 6 problemas — comentário de invariante do §4.3 apagado sem relação com o trabalho da sprint, `disabled_features` da tabela `error.rs` ignorado pela UI (Whisper e "Abrir URL" guiavam-se por `matches!` ad-hoc), gap de validação de esquema do §4.27 introduzido por o modal de URL ter sido implementado como funcional em vez de apenas visível (a integração real com yt-dlp continua a ser âmbito da Sprint_05), `_ =>` genérico no `VadError::action()` que derrotava a exaustividade do compilador, teste que mutava `$PATH` global entre threads, e uma asserção tautológica sempre-verdadeira num teste. Detalhe completo em `Sprint_02.md` §"Correções de revisão".
...
- **Leituras de propriedades do HUD não seguem o padrão do `time-pos` (§5/tarefa 6 da Sprint_01).** `speed()`, `volume()`, `is_muted()` e `ab_loop_status()` são lidos do mpv em todos os frames enquanto o HUD está visível (só as faixas de áudio/legendas são cacheadas com throttling de 3s). Não é um bug — cada leitura é uma chamada de propriedade barata, não um evento de alta frequência como `time-pos` — mas é uma inconsistência a rever se a Sprint_15 (`theme.rs`/polish) tocar neste ficheiro.
- **Validação de esquema de URL (§4.27) ainda mínima.** A Sprint_02 acrescentou apenas o allowlist de esquema (`http://`/`https://`/`rtsp://`) no submit do modal, o suficiente para fechar o gap introduzido por adiantar a funcionalidade de URL. A validação completa contra SSRF/endpoints internos e a integração real com `yt-dlp` continuam na Sprint_05, como planeado.
